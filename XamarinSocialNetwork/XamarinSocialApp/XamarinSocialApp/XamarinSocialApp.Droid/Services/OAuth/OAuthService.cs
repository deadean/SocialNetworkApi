...
namespace XamarinSocialApp.Droid.Services.OAuth
{
	public class OAuthService : IOAuthService
	{
		...
		static string mvUserName;
		public string UserName
		{
			get { return mvUserName; }
			set { mvUserName = value; }
		}
		...
		public async Task<string> GetUserInfoRequest(string uid)
		{
			...
			var msg = JsonConvert.DeserializeObject<VkData.VkUsers>(responseText);
			
			string userN = String.Format("{0} {1}", msg.response.First().first_name, msg.response.First().last_name);
			return userN;
		}


		public async Task<IList<string>> GetDialogs()
		{
			...
			request.Parameters.Add("count", "10");
			...
			{
				if (item.Message.Body == String.Empty)
					continue;

				string name = await Task<string>.Run(() => this.GetUserInfoRequest(item.Message.UserId).Result);

				Items.Add(name + " | " + item.Message.Body);
			}
